...
///    call site marks them, so nothing but a scan would notice one going missing. The editor's own
///    helpers that take a `LocalizedStringKey` — `header("…")`, `thresholdField("…")` — are the same
///    case and are listed with them.
///
...
enum LocalizableScan {
    ...
    ///
    /// Every literal in one of these calls is taken as a key, not only the first, because a ternary
    /// between two literals — `Text(isEmpty ? "…" : "…")` — makes both of them keys.
    private static let callShapes = [
        "String(", "NSLocalizedString(",
        "Text(", "Toggle(", "Button(", "Picker(", "LabeledContent(", "Label(", "Section(",
        "Stepper(", "TextField(", "Link(", "Menu(", "ContentUnavailableView(", ".help(", ".alert(",
        // The editor's helpers whose title parameter is a `LocalizedStringKey`.
        "header(", "alertChoice(", "thresholdField(", "MetricPicker(",
    ]
    ...
    private static let skippedLabels = [
        "verbatim:", "systemName:", "systemSymbolName:", "systemImage:", "accessibilityDescription:",
        ...
        for shape in callShapes {
            for start in ranges(of: shape, in: skeleton) {
                ...
                for literal in inCall {
                    ...
                    // An explicit `localized:` argument is a key whatever it
                    // starts with -- "24-hour, minute, second" is one.
                    guard let first = literal.pattern.first,
                          literal.label == "localized:"
        ...
        while index < characters.count {
            ...
            while index < characters.count {
                if characters[index] == "\\", index + 1 < characters.count {
                    ...
                    // `\u{2026}`: one scalar, written in hex. The source spells
                    // ellipses and curly quotes this way, and the catalogue holds
                    // the characters themselves.
                    if characters[index + 1] == "u", index + 2 < characters.count,
                       characters[index + 2] == "{",
                       let close = characters[(index + 3)...].firstIndex(of: "}"),
                       let value = UInt32(String(characters[(index + 3)..<close]), radix: 16),
